...
func TestRehandshakingRace(t *testing.T) {
	...
	r.Log("Renew certificate and spin until their sees my new certificate")
	_, _, myNextPrivKey, myNextPEM := newTestCert(ca, caKey, "me", time.Now(), time.Now().Add(5*time.Minute), myVpnIpNet, nil, []string{"my new group"})
	...
	for {
		assertTunnel(t, myVpnIpNet.IP, theirVpnIpNet.IP, myControl, theirControl, r)
		myCertInThem := theirControl.GetHostInfoByVpnIp(iputil.Ip2VpnIp(myVpnIpNet.IP), false)
		theirCertInMe := myControl.GetHostInfoByVpnIp(iputil.Ip2VpnIp(theirVpnIpNet.IP), false)

		t.Log(theirCertInMe.Cert.Details.Groups, myCertInThem.Cert.Details.Groups)
		_, theirNewGroup := theirCertInMe.Cert.Details.InvertedGroups["their new group"]
		_, myNewGroup := myCertInThem.Cert.Details.InvertedGroups["my new group"]
		if theirNewGroup && myNewGroup {
			t.Log(theirCertInMe.Cert.Details.Groups, myCertInThem.Cert.Details.Groups)
			break
		}
...
func TestRehandshakingPrimary(t *testing.T) {
	//TODO: note that this is a rehandshake and record the old index so it can be closed later
	ca, _, caKey, _ := newTestCaCert(time.Now(), time.Now().Add(10*time.Minute), []*net.IPNet{}, []*net.IPNet{}, []string{})
	myControl, myVpnIpNet, myUdpAddr, myConfig := newSimpleServer(ca, caKey, "me  ", net.IP{10, 0, 0, 2}, nil)
	theirControl, theirVpnIpNet, theirUdpAddr, theirConfig := newSimpleServer(ca, caKey, "them", net.IP{10, 0, 0, 1}, nil)
	...
	assert.NoError(t, err)
	theirConfig.ReloadConfigString(string(rc))
	theirControl.ReHandshake(iputil.Ip2VpnIp(myVpnIpNet.IP))

	for {
		assertTunnel(t, myVpnIpNet.IP, theirVpnIpNet.IP, myControl, theirControl, r)
		theirCertInMe := myControl.GetHostInfoByVpnIp(iputil.Ip2VpnIp(theirVpnIpNet.IP), false)

		t.Log(theirCertInMe.Cert.Details.Groups)
		_, theirNewGroup := theirCertInMe.Cert.Details.InvertedGroups["their new group"]
		if theirNewGroup {
			t.Log(theirCertInMe.Cert.Details.Groups)
			break
	...
	r.Log("Renew my certificate and spin until they see it")
	_, _, myNextPrivKey, myNextPEM := newTestCert(ca, caKey, "me", time.Now(), time.Now().Add(5*time.Minute), myVpnIpNet, nil, []string{"my new group"})
	...
	assert.NoError(t, err)
	myConfig.ReloadConfigString(string(rc))
	// Force a re-handshake now
	myControl.ReHandshake(iputil.Ip2VpnIp(theirVpnIpNet.IP))

	for {
		assertTunnel(t, myVpnIpNet.IP, theirVpnIpNet.IP, myControl, theirControl, r)
		myCertInThem := theirControl.GetHostInfoByVpnIp(iputil.Ip2VpnIp(myVpnIpNet.IP), false)

		t.Log(myCertInThem.Cert.Details.Groups)
		_, myNewGroup := myCertInThem.Cert.Details.InvertedGroups["my new group"]
		if myNewGroup {
			t.Log(myCertInThem.Cert.Details.Groups)
			break
	...
	assert.Len(t, myFinalHostmapIndexes, 1)
	assert.Len(t, theirFinalHostmapIndexes, 1)
	t.Log(myFinalHostmapHosts[0].Cert.Details.Groups, theirFinalHostmapHosts[0].Cert.Details.Groups)
...
//TODO: test rehandshaking through relays
